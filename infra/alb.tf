...
resource "aws_lb_target_group" "oracle" {
  ...
  health_check {
    ...
    timeout             = 5
    interval            = 30
    matcher             = "200,503" # 503 is ok while enclave boots
  }
...
# HTTP Listener — always forward (HTTPS listener handles SSL when cert exists)
...
# Output ALB DNS
output "alb_dns_name" {
  description = "ALB DNS name — use this to access the pull API"
